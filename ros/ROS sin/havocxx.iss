...
function main()
{
	...
	while 1
	{
		while ${Me.InCombat}==TRUE
		{
			...
			{
				if (${Me.Ability[id,3633058267].TimeUntilReady}<=4 || ${Me.Ability[id,2305497779].TimeUntilReady}==0) && ${Me.Ability[id,3051745403].TimeUntilReady}<=7 && (${Me.Ability[id,866089504].TimeUntilReady} <=7) && (${Me.Ability[id,3633058267.TimeUntilReady} <=4) && (((${Target.Type.Equal["NamedNPC"]} || ${Target.Type.Equal["NPC"]}) && ${TargetDist}<=5.5) || ((${Target.Target.Type.Equal["NamedNPC"]} || ${Target.Target.Type.Equal["NPC"]}) && ${TargetTargetDist}<=5))
				{
